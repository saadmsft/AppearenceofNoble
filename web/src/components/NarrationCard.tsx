...
export function NarrationCard({ row, language, saved, onSave, onRead }: CardProps) {
  const t = (key: Parameters<typeof translate>[1], values?: Record<string, string>) => translate(language, key, values)
  return <article className="narration-card" aria-labelledby={`title-${row.id}`} data-entry={row.id}>
    ...
    <div className="card-bottom">
      ...
      <Button variant="ghost" size="icon" className={saved ? 'is-saved' : ''} onClick={onSave} aria-pressed={saved} aria-label={t(saved ? 'unsave' : 'save', { title: row.title[language] })}>
