...
test('the project home opens both collections without revealing narration cards', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Explore Character', exact: true }).click()
  await expect(page.getByRole('heading', { level: 1 })).toContainText('Known through character.')
...
test('Urdu project navigation retains the manuscript layout and correct collection', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'اخلاقِ نبوی کا مطالعہ', exact: true }).click()
  await expect(page.locator('.journey-chapter')).toHaveCount(8)
  expect(new URL(page.url()).searchParams.get('shelf')).toBe('character')
